...
def run():
    ...
    url = 'http://dota2.vpgame.com/api/dota2LeagueMatch-rank.html?pageSize=10&showType=page' +\
          '&type=overview&season_id=554&_=1449545032706&DotaLeagueSeasonMemberStatistics_page=1'
    ...
    for i in range(1, total_page):
        url = 'http://dota2.vpgame.com/api/dota2LeagueMatch-rank.html?pageSize=10&showType=page' +\
              '&type=overview&season_id=554&_=1449545032706&DotaLeagueSeasonMemberStatistics_page='+str(i)
        ht = requests.get(url, headers=head).content
        jsDict = json.loads(ht)
        jsData = jsDict['data']
        for each in jsData:
